...
BEGIN
   ...
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 1;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 1;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 5, SYSDATE));


   -- Ocena 2: Uczeń 2, Matematyka
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 2;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 2;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 4, SYSDATE));


   -- Ocena 3: Uczeń 3, Informatyka
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 3;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 3;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 3, SYSDATE));


   -- Ocena 4: Uczeń 4, Fizyka
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 4;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 4;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 6, SYSDATE));


   -- Ocena 5: Uczeń 5, Chemia
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 5;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 5;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 2, SYSDATE));


   -- Ocena 6: Uczeń 6, Biologia
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 6;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 6;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 5, SYSDATE));


   -- Ocena 7: Uczeń 7, Historia
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 7;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 7;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 4, SYSDATE));


   -- Ocena 8: Uczeń 8, Geografia
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 8;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 8;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 3, SYSDATE));


   -- Ocena 9: Uczeń 9, Wychowanie Fizyczne
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 9;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 9;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 6, SYSDATE));


   -- Ocena 10: Uczeń 10, Język Angielski
   SELECT REF(u) INTO v_uczen_ref FROM C##SZKOLA.uczniowie u WHERE u.id = 10;
   SELECT REF(p) INTO v_przedmiot_ref FROM C##SZKOLA.przedmioty p WHERE p.przedmiot_id = 10;
   INSERT INTO C##SZKOLA.oceny VALUES (C##SZKOLA.ocena_type(v_uczen_ref, v_przedmiot_ref, 5, SYSDATE));
END;
/
